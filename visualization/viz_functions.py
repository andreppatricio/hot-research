...
def barplot_ngrams(spark, query, x, y, title=None):
    if title is None:
        title = "This week's most used keywords"
...
def lineplot_ngrams(spark_session, query, x, y, hue, title=None):
    if title is None:
        title = "Evolution of keywords"
    results = spark_session.sql(query)
    results_df = results.toPandas()
...
def barplot_journals(spark, query, x, y, title=None):
    if title is None:
        title = "Journals with most publications"
...
def barplot_authors(spark, query, x, y, title = None):
    if title is None:
        title = "Authors with most publications"
    results_df = spark.sql(query).toPandas()
    values = results_df[y]
    normalized_values = (values - min(values)) / (max(values) - min(values))
    # Create a custom color palette based on the normalized values
